...
Generated by `npm run generate:workbook` (deterministic, seeded PRNG):

- **33 skills** (S_001–S_033) forming the NCERT knowledge graph, Grade 10
  topics chaining down to Grade 5 root nodes
- **304 questions** (264 model + 40 equation twins; 40 word problems), 8 per
  skill across easy/medium/hard
- **912 answer traps** — every wrong option classified
  ...
  `flag_review`)
- **Q-Matrix** (490 links) and **5-dimension tags** per question

The admin **Question Bank → Excel Import** panel accepts any workbook in this
format: validate (detailed row/column error report + preview diff) → confirm →
bulk upsert.
